...
/***/ function(module, exports, __webpack_require__) {
	...
	var FellowshipTablePage = function (_AdminBasePage) {
	    _inherits(FellowshipTablePage, _AdminBasePage);

	    function FellowshipTablePage() {
	        _classCallCheck(this, FellowshipTablePage);

	        return _possibleConstructorReturn(this, (FellowshipTablePage.__proto__ || Object.getPrototypeOf(FellowshipTablePage)).apply(this, arguments));
	    }

	    _createClass(FellowshipTablePage, [{
	        key: "init",
	        value: function init() {
	            var table = this;
	        ...
	        value: function enable(name) {
	            new _dialog2.default("激活团契", "激活团契后团契和团契所属的文章会重新在网站显示，确认要激活吗？", function () {
	                ...
	                $.ajax({
	                    ...
	                    success: function success(status) {
	                        if ("success" == status) {
	                            ...
	                            new _dialog2.default("激活团契", "激活失败，原因：" + status, function () {}).error();
	                        }
	                    },
	                    error: function error() {
	                        new _dialog2.default("激活团契", "激活失败", function () {}).error();
	                    }
	                });
	            }).confirm();
	        }
	        ...
	        value: function disable(name) {
	            new _dialog2.default("禁用团契", "禁用团契后团契和团契所属的文章将不会在网站显示，确认要禁用吗？", function () {
	                var formData = new FormData();
	                formData.append("_csrf", $(".ui.admin.user.form input[name='_csrf']").val());
	                formData.append("name", name);
	                ...
	                $.ajax({
	                    ...
	                    success: function success(status) {
	                        if ("success" == status) {
	                            ...
	                            new _dialog2.default("禁用团契", "禁用失败，原因：" + status, function () {}).error();
	                        }
	                    },
	                    error: function error() {
	                        new _dialog2.default("禁用团契", "禁用失败", function () {}).error();
	                    }
	                });
	            }).confirm();
	        }
	    ...
	    return FellowshipTablePage;
	}(_joshuaAdminBasePage2.default);

	exports.default = FellowshipTablePage;
