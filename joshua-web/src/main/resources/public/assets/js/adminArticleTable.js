...
/***/ function(module, exports, __webpack_require__) {
	...
	var AdminArticleTablePage = function (_AdminBasePage) {
	    _inherits(AdminArticleTablePage, _AdminBasePage);

	    function AdminArticleTablePage() {
	        _classCallCheck(this, AdminArticleTablePage);

	        return _possibleConstructorReturn(this, (AdminArticleTablePage.__proto__ || Object.getPrototypeOf(AdminArticleTablePage)).apply(this, arguments));
	    }

	    _createClass(AdminArticleTablePage, [{
	        key: "init",
	        value: function init() {
	            var articleTable = this;
	        ...
	        value: function enable(id) {
	            new _dialog2.default("激活文章", "激活文章后会重新在网站显示，确认要激活吗？", function () {
	                ...
	                $.ajax({
	                    ...
	                    success: function success(status) {
	                        if ("success" == status) {
	                            ...
	                            new _dialog2.default("激活文章", "激活失败，原因：" + status, function () {}).error();
	                        }
	                    },
	                    error: function error() {
	                        new _dialog2.default("激活文章", "激活失败", function () {}).error();
	                    }
	                });
	            }).confirm();
	        }
	        ...
	        value: function disable(id) {
	            new _dialog2.default("禁用文章", "禁用文章后不会在网站显示，确认要禁用吗？", function () {
	                var formData = new FormData();
	                formData.append("_csrf", $(".ui.admin.user.form input[name='_csrf']").val());
	                formData.append("id", id);
	                ...
	                $.ajax({
	                    ...
	                    success: function success(status) {
	                        if ("success" == status) {
	                            ...
	                            new _dialog2.default("禁用文章", "禁用失败，原因：" + status, function () {}).error();
	                        }
	                    },
	                    error: function error() {
	                        new _dialog2.default("禁用文章", "禁用失败", function () {}).error();
	                    }
	                });
	            }).confirm();
	        }
