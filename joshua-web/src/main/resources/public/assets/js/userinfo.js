...
/***/ function(module, exports, __webpack_require__) {
	...
	var UserInfoPage = function (_BasePage) {
	    _inherits(UserInfoPage, _BasePage);

	    function UserInfoPage() {
	        _classCallCheck(this, UserInfoPage);

	        return _possibleConstructorReturn(this, (UserInfoPage.__proto__ || Object.getPrototypeOf(UserInfoPage)).apply(this, arguments));
	    }
	        ...
	        value: function init() {
	            $(".ui.user.password.submit.button").on("click", function () {
	                ...
	                if (newPassword.length < 8) {
	                    new _dialog2.default("修改密码", "新的密码长度不能少于8位").warning();
	                    return;
	                }

	                if (newPassword != confirmPassword) {
	                    new _dialog2.default("修改密码", "两次密码输入不一致").warning();
	                    return;
	                }
	                ...
	                $.ajax({
	                    ...
	                    success: function success(status) {
	                        if ("success" == status) {
	                            new _dialog2.default("修改密码", "修改密码成功", function () {
	                                window.location.reload();
	                            }).message();
	                        } else {
	                            new _dialog2.default("修改密码", "修改密码失败，原因:" + status, function () {}).error();
	                        }
	                    },
	                    error: function error() {
	                        new _dialog2.default("修改密码", "修改密码失败", function () {}).error();
	                    }
	                });
	            });
	        }
