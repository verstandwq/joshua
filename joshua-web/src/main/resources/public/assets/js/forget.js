...
/***/ function(module, exports, __webpack_require__) {
	...
	var ForgetPage = function (_BasePage) {
	    _inherits(ForgetPage, _BasePage);

	    function ForgetPage() {
	        _classCallCheck(this, ForgetPage);

	        return _possibleConstructorReturn(this, (ForgetPage.__proto__ || Object.getPrototypeOf(ForgetPage)).apply(this, arguments));
	    }
	                ...
	                if (username && email) {
	                    ...
	                    $.ajax({
	                        ...
	                        success: function success(status) {
	                            if ("success" == status) {
	                                new _dialog2.default("重置密码", "重置密码成功，密码已经发送到邮箱：" + email, function () {
	                                    window.location = "login";
	                                }).message();
	                            } else {
	                                new _dialog2.default("重置密码", "重置密码失败，原因：" + status).error();
	                            }
	                        },
	                        error: function error() {
	                            new _dialog2.default("重置密码", "重置密码失败", function () {}).error();
	                        }
	                    });
	                } else {
	                    new _dialog2.default("信息不正确", "必须填写登录名和邮箱").message();
	                }
	            });
	        }
	    }]);
