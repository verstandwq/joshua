...
/***/ function(module, exports, __webpack_require__) {
	...
	var FellowshipPage = function (_AdminBasePage) {
	    _inherits(FellowshipPage, _AdminBasePage);

	    function FellowshipPage() {
	        _classCallCheck(this, FellowshipPage);

	        return _possibleConstructorReturn(this, (FellowshipPage.__proto__ || Object.getPrototypeOf(FellowshipPage)).apply(this, arguments));
	    }

	    _createClass(FellowshipPage, [{
	        key: "init",
	        value: function init() {
	            var fellowship = this;
	        ...
	        key: "transfer",
	        value: function transfer(name, username) {
	            ...
	            $.ajax({
	                ...
	                success: function success(status) {
	                    if ("success" == status) {
	                        ...
	                        new _dialog2.default("转移团契", "转移失败，原因：" + status, function () {}).error();
	                    }
	                },
	                error: function error() {
	                    new _dialog2.default("转移团契", "转移失败", function () {}).error();
	                }
	            });
	        }
	    }, {
	        key: "addAdmin",
	        value: function addAdmin(name, username) {
	            var formData = new FormData();
	            formData.append("_csrf", $(".ui.admin.user.form input[name='_csrf']").val());
	            formData.append("name", name);
	            formData.append("username", username);
	            ...
	            $.ajax({
	                ...
	                success: function success(status) {
	                    if ("success" == status) {
	                        ...
	                        new _dialog2.default("添加管理员", "添加管理员失败，原因：" + status, function () {}).error();
	                    }
	                },
	                error: function error() {
	                    new _dialog2.default("添加管理员", "添加管理员失败", function () {}).error();
	                }
	            });
	        }
	    }, {
	        key: "removeAdmin",
	        value: function removeAdmin(name, username) {
	            new _dialog2.default("移除管理员", "确定要移除管理员" + username + "?", function () {
	                var formData = new FormData();
	                formData.append("_csrf", $(".ui.admin.user.form input[name='_csrf']").val());
	                formData.append("name", name);
	                formData.append("username", username);
	                ...
	                $.ajax({
	                    ...
	                    success: function success(status) {
	                        if ("success" == status) {
	                            ...
	                            new _dialog2.default("移除管理员", "移除管理员失败，原因：" + status, function () {}).error();
	                        }
	                    },
	                    error: function error() {
	                        new _dialog2.default("移除管理员", "移除管理员失败", function () {}).error();
	                    }
	                });
	            }).confirm();
	        }
	...
	exports.default = FellowshipPage;
